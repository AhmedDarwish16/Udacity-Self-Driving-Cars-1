...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// todo - your code here
    //The idea here is to caclulate the sum pf the probability 
    //and then divide each cellby this sum to get the normalizer
    int rows_number= grid.size();
    int cols_number=grid[0].size();
    float sumCells=0;
    vector<vector<float> > newGrid(rows_number,vector<float> (cols_number,0));
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    //Defining the window Matrix by using c++11
